...
import { CAMPUS_CAT_DISPLAY_NAME, studyGear } from '../inventory/StudyGearStore'
...
type SeatPointerTarget = Readonly<{
  seat: ImageRoomSeat
...
export class ImageRoomScene extends Phaser.Scene {
  ...
  #seatPointerTargets: readonly SeatPointerTarget[] = []
  #interactionObstacles: readonly (readonly WorldPoint[])[] = []
  ...
  #pointerIntentTimer: number | null = null
  ...
  #poolDiveRequestHandler = (): void => {
    if (this.#poolDiveRequestPending) return
    if (this.#roomId !== 'sports-center') {
      window.dispatchEvent(new CustomEvent('radiotedu:study-pool-dive-failed'))
      return
    }
    this.#poolDiveRequestPending = true
    const requestedRoom = this.#roomId
    void this.#walkToNode('dive-queue').then(() => {
      if (this.#roomId !== requestedRoom || this.#currentNodeId !== 'dive-queue') {
        window.dispatchEvent(new CustomEvent('radiotedu:study-pool-dive-failed'))
        return
      }
      window.dispatchEvent(new CustomEvent('radiotedu:study-pool-dive-ready'))
    }).catch(() => window.dispatchEvent(new CustomEvent('radiotedu:study-pool-dive-failed')))
      .finally(() => { this.#poolDiveRequestPending = false })
  }
  ...
  create(): void {
    ...
    window.addEventListener('radiotedu:study-pool-dive-request', this.#poolDiveRequestHandler)
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      if (this.#pointerIntentTimer !== null) window.clearTimeout(this.#pointerIntentTimer)
      this.#pointerIntentTimer = null
      ...
      window.removeEventListener('radiotedu:study-pool-dive-request', this.#poolDiveRequestHandler)
    })
  ...
  #clearRoomObjects(): void {
    ...
    delete document.documentElement.dataset.poolDiveStation
  }
  ...
  #renderRoom(roomId: ImageRoomId): void {
    ...
    this.#seatPointerTargets = this.#room.seats.map((seat) => ({
      seat,
      hitArea: resolveSeatGeometry(this.#room, seat).hitArea.map((point) => roomPointToPixel(this.#room, point)),
    }))
    this.#interactionObstacles = roomInteractionObstacles(this.#room)
    ...
    const catSeatObstacles = this.#seatPointerTargets.map((target) => target.hitArea)
    ...
    if (roomId === 'sports-center') this.#createPoolDiveStation()
  ...
  #createCampusCats(): void {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const name = CAMPUS_CAT_DISPLAY_NAME
      ...
      const shadow = this.add.ellipse(
        ...
        .setDepth(depth + 42)
      const sprite = this.add.sprite(pixel.x, pixel.y, `campus-cat:${variant}`)
  ...
  async #startCampusCatWander(cat: CampusCat): Promise<void> {
    if (!cat.sprite.active || cat.roomId !== this.#roomId || cat.walking) return
    ...
    const candidates = roomCatPatrolPoints(this.#room).filter((point) => (
      point.z === cat.z
      && point.id !== cat.nodeId
      && !occupiedSeatAnchors.some((anchor) => Math.hypot(anchor.x - point.x, anchor.y - point.y) < 90)
      && navigationField.isWalkable(point, point.z)
    ))
    Phaser.Utils.Array.Shuffle(candidates)

    let selected: Readonly<{ target: (typeof candidates)[number]; route: readonly WorldPoint[] }> | null = null
    for (const target of candidates) {
      const route = await navigationField.findPathAsync(
        { x: cat.sprite.x, y: cat.sprite.y },
        target,
        cat.z,
        { budgetMs: 2, shouldContinue },
      )
    ...
    if (!selected) {
      if (shouldContinue()) this.#scheduleCampusCatWander(cat)
      return
    }

    const { target, route } = selected
  ...
  #createWorldActors(): void {
    for (const [actorId, actor] of Object.entries(this.#room.actors)) {
      ...
      if (actorId === 'spark') {
        ...
        badge.fillStyle(0x102d31, 0.94).fillCircle(1, 2, 19)
        badge.lineStyle(2, 0xc4fff6, 0.92).fillStyle(0x55d9cc, 1).fillPoints(sparkPoints, true)
        badge.strokePoints(sparkPoints, true, true)
        badge.fillStyle(0xeafffb, 0.94).fillPoints([
          new Phaser.Math.Vector2(0, -9), new Phaser.Math.Vector2(3, -3), new Phaser.Math.Vector2(9, 0),
          new Phaser.Math.Vector2(3, 3), new Phaser.Math.Vector2(0, 9), new Phaser.Math.Vector2(-3, 3),
          new Phaser.Math.Vector2(-9, 0), new Phaser.Math.Vector2(-3, -3),
        ], true)
        badge.fillStyle(0xec2432, 1).fillCircle(0, 0, 2.5)
      } else {
        const rockPoints = [
          new Phaser.Math.Vector2(-17, -8), new Phaser.Math.Vector2(-10, -16), new Phaser.Math.Vector2(7, -15),
          new Phaser.Math.Vector2(17, -7), new Phaser.Math.Vector2(15, 10), new Phaser.Math.Vector2(7, 16),
          new Phaser.Math.Vector2(-11, 14), new Phaser.Math.Vector2(-18, 5),
        ]
        badge.fillStyle(0x15191c, 0.9).fillEllipse(1, 15, 36, 10)
        badge.lineStyle(2, 0x171b1e, 1).fillStyle(0x555b60, 1).fillPoints(rockPoints, true)
        badge.strokePoints(rockPoints, true, true)
        badge.fillStyle(0x747b80, 1).fillTriangle(-10, -12, 7, -13, -4, -2)
        badge.fillStyle(0x3c4247, 1).fillTriangle(-15, -6, -4, -2, -10, 11)
        badge.fillStyle(0x292f33, 1).fillCircle(3, 4, 7)
        badge.lineStyle(1, 0x9da5a9, 0.88).strokeCircle(3, 4, 7)
        badge.fillStyle(0x171c1f, 1).fillCircle(3, 4, 3)
        badge.fillStyle(0xec2432, 1).fillCircle(10, -7, 2)
        badge.fillStyle(0xd1dbd8, 0.9).fillRect(-8, 5, 2, 3)
        badge.fillRect(-5, 2, 2, 6)
        badge.fillRect(-2, -1, 2, 9)
      }
      ...
      if (window.matchMedia?.('(prefers-reduced-motion: reduce)').matches !== true) {
        this.tweens.add({
          ...
          scaleX: 1.045,
          scaleY: 1.045,
          duration: actorId === 'spark' ? 980 : 1_240,
          ease: 'Sine.InOut',
          yoyo: true,
          repeat: -1,
        })
  ...
  #createPoolDiveStation(): void {
    const board = roomPointToPixel(this.#room, { x: 50, y: 70.5 })
    const container = this.add.container(board.x, board.y - 52)
      .setDepth(imageRoomActorDepth({ y: 70.5, z: 0 }, 30))
    const panel = this.add.graphics()
    panel.fillStyle(0x0d1b20, 0.94).fillRoundedRect(-78, -26, 156, 52, 5)
    panel.lineStyle(2, 0x89ddbc, 1).strokeRoundedRect(-78, -26, 156, 52, 5)
    panel.fillStyle(0xec2432, 1).fillTriangle(-65, -7, -54, 0, -65, 7)
    const title = this.add.text(-45, -15, 'DEEP DIVE', {
      color: '#f7fff9', fontFamily: 'Segoe UI, sans-serif', fontSize: '13px', fontStyle: 'bold',
    })
    const hint = this.add.text(-45, 4, 'PLAY AT THE BOARD', {
      color: '#9fd7c1', fontFamily: 'Segoe UI, sans-serif', fontSize: '8px', fontStyle: 'bold',
      letterSpacing: 1,
    })
    container.add([panel, title, hint]).setSize(180, 76).setInteractive({ useHandCursor: true })
    container.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      const target = pointer.event.target
      if (target instanceof Element && target.closest('[data-study-ui]')) return
      pointer.event.stopPropagation()
    ...
    if (window.matchMedia?.('(prefers-reduced-motion: reduce)').matches !== true) {
      this.tweens.add({ targets: container, y: container.y - 5, duration: 950, ease: 'Sine.InOut', yoyo: true, repeat: -1 })
    }
    this.#roomObjects.push(container)
  ...
  #setSeatForeground(seat: ImageRoomSeat | null): void {
    ...
    if (this.#roomId === 'auditorium') {
      ...
      const anchor = roomPointToPixel(this.#room, geometry.actorAnchor)
      const maskSource = this.make.graphics({ x: 0, y: 0, add: false } as never)
      // Each authored polygon follows the visible chair shell. Restoring that
      // shape, rather than a rectangular room crop, keeps the bent seated body
      // behind the chair while preserving the surrounding aisle and shoulders.
      maskSource.fillStyle(0xffffff)
      if (seat.foregroundMask?.length) {
        maskSource.fillPoints(seat.foregroundMask.map((point) => new Phaser.Math.Vector2(
          (point.x / 100) * this.#room.image.width,
          (point.y / 100) * this.#room.image.height,
        )), true)
      } else {
        maskSource.fillRect(anchor.x - 28, anchor.y - 24, 56, 36)
      }
      const image = this.add.image(0, 0, `room:${this.#roomId}`)
        .setOrigin(0)
        .setDepth(imageRoomActorDepth(geometry.actorAnchor, 20))
  ...
  async #routeToPoint(
  ...
  ): Promise<readonly Readonly<{ id: string; x: number; y: number; z: number }>[]> {
    const start = { x: this.#avatar.x, y: this.#avatar.y }
    const direct = this.#currentZ === targetZ
      ? await this.#navigationField.findPathAsync(start, target, targetZ, { budgetMs: 4, shouldContinue })
      : []
    if (!shouldContinue()) return []
    if (direct.length > 0) {
    ...
    for (let index = 0; index < legacyIds.length; index += 1) {
      ...
      if (previous.z === node.z) {
        const fieldSection = await this.#navigationField.findPathAsync(previous, pixel, node.z, { budgetMs: 4, shouldContinue })
        if (!shouldContinue()) return []
    ...
    if (Math.hypot(tail.x - target.x, tail.y - target.y) > 1) {
      const finalSection = await this.#navigationField.findPathAsync(tail, target, targetZ, { budgetMs: 4, shouldContinue })
      if (!shouldContinue()) return []
  ...
  async #walkToPoint(
    ...
    const routePoints = [...await this.#routeToPoint(
      target,
      targetZ,
      targetNodeId,
      () => this.#activity.isCurrent(activityToken),
    )]
    if (!this.#activity.isCurrent(activityToken)) return
  ...
  #createAuditoriumEventScreen(): void {
    document.documentElement.dataset.auditoriumScreen = 'radiotedu'
    const screen = this.add.image(0, 0, 'auditorium:radiotedu-screen')
      .setOrigin(0)
      .setDepth(-90_000)
      .setAlpha(0.98)
  ...
  async #sit(seat: ImageRoomSeat, activityToken: ActivityToken): Promise<void> {
    ...
    const facing = usesAcceptedSeatAsset
      ? seat.facing
      : this.#roomId === 'library' || this.#roomId === 'chim-alan' || this.#roomId === 'grass-amphitheatre'
  ...
  #bindPointerMovement(): void {
    this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      // ScaleManager already tracks the rendered canvas size. Reading the DOM
      // bounds here forced a synchronous layout on every tap and made a burst
      // of mobile input visibly hitch even though route work was coalesced.
      const displayWidth = Math.max(1, this.scale.displaySize.width)
      const displayHeight = Math.max(1, this.scale.displaySize.height)
      const seatHitSlop = screenSlop * Math.max(
        this.cameras.main.worldView.width / displayWidth,
        this.cameras.main.worldView.height / displayHeight,
      )
      ...
      if (this.#pointerIntentTimer !== null) window.clearTimeout(this.#pointerIntentTimer)
      // Treat a burst of mouse/touch input as one latest-intent action. Route
      // planning is deliberately trailing and short enough to feel immediate
      // for a single click, while preventing 18 rapid clicks from running 18
      // synchronous A* searches and freezing the render loop.
      this.#pointerIntentTimer = window.setTimeout(() => {
        this.#pointerIntentTimer = null
  ...
  #processPointerIntent(world: TouchWorldPoint, uiConsumed: boolean, seatHitSlop: number): void {
    ...
    const hitSeat = pointsAtPlayer
      ? null
      : resolveSeatHitTarget(world, this.#seatPointerTargets, seatHitSlop)?.seat ?? null
    ...
    const pointsAtFurniture = !hitSeat
      && this.#interactionObstacles.some((polygon) => pointInPolygon(world, polygon))
    const walkable = uiConsumed || pointsAtPlayer || hitSeat !== null
      ? false
      : !pointsAtFurniture && targetZ !== null && this.#navigationField.isWalkable(world, targetZ) && (
  ...
  #syncHud(): void {
    ...
    if (window.matchMedia('(max-width: 700px)').matches) {
      const selectedRoom = document.querySelector<HTMLButtonElement>(`[data-room-id="${this.#roomId}"]`)
      globalThis.requestAnimationFrame(() => selectedRoom?.scrollIntoView({ block: 'nearest', inline: 'center' }))
    }
    document.querySelectorAll<HTMLButtonElement>('[data-wearable-id]').forEach((button) => {
  ...
  #exposeDebugApi(): void {
    window.__STUDY_GAME_APP__ = {
      ...
      tapTargets: () => {
        ...
        return {
          activities: this.#roomId === 'sports-center' ? [(() => {
            const world = roomPointToPixel(this.#room, { x: 50, y: 70.5 })
            const marker = { x: world.x, y: world.y - 52 }
            return { id: 'pool-dive' as const, world: marker, screen: screen(marker) }
          })()] : [],
          floor: this.#navigationField.layerIds().flatMap((z) => (
            this.#navigationField.samples(z, Math.max(4, Math.floor(64 / this.#navigationField.cellSize))).map((world, index) => ({
              id: `floor:${z}:${index}`,
              z,
              world,
              screen: screen(world),
      ...
      snapshot: () => ({
        ...
        animationFrame: this.#avatarAnimationFrame % ACTION_FRAMES[this.#avatarController.action],
        ...
        remotePlayers: this.#socialObjects.flatMap((object) => {
          if (!(object instanceof Phaser.GameObjects.Container)) return []
          const userId = object.getData('avatarUserId') as string | undefined
          if (!userId) return []
          return [{
            userId,
            displayName: object.getData('avatarDisplayName') as string,
            position: { x: object.x, y: object.y },
            appearance: object.getData('avatarAppearance') as AvatarAppearance,
            layerTextures: object.getData('avatarLayerTextures') as string[],
          }]
        }),
        camera: {

...
import { ArrowRight, Armchair, BookOpen, CalendarDays, Check, ChevronDown, ChevronRight, Clock3, Coins, createIcons, EllipsisVertical, Flame, Hand, HelpCircle, Keyboard, LockKeyhole, LogIn, LogOut, Map, MapPin, MessageCircle, Monitor, Pause, Play, Radio, Send, Settings, ShieldCheck, Shirt, Sparkles, Star, Trophy, UserPlus, UserRound, UsersRound, Volume2, Waves, X } from 'lucide'
...
import { GOLD_STORE_ITEMS, type GoldStoreItem } from './inventory/GoldStoreCatalog'
...
const WORLD_ICONS = Object.freeze({
  Armchair, BookOpen, CalendarDays, Check, ChevronDown, ChevronRight, Coins, EllipsisVertical, Hand, HelpCircle,
  ...
  UserRound, UsersRound, Volume2, Waves, X,
})
...
import { chooseRoomInstance } from './ui/RoomInstanceSelector'
...
const ROOM_SUMMARIES: Readonly<Record<StudyRoomId, string>> = Object.freeze({
  ...
  'chim-alan': 'Campus entrance · outdoor meet-up',
  'grass-amphitheatre': 'Grass amphitheatre · outdoor study',
  'sports-center': 'Swimming · server-scored Deep Dive',
  auditorium: 'Talks · live campus events',
  'learning-lab': 'Computer lab · focused study seats',
})
...
async function bootStudy(secureBridge: ActiveStudyBridge | null, entryConfig: ReturnType<typeof resolveStudyEntry>) {
  ...
  const launchWorld = async (roomId: ImageRoomId, preferredInstanceId?: string | null) => {
    if (worldStarted || worldStarting) return
    worldStarting = true
    try {
      if (preferredInstanceId) {
        await adapter.enterRoom(roomId, IMAGE_ROOMS[roomId].spawnNodeId, preferredInstanceId)
      }
    } catch {
      worldStarting = false
      const status = document.querySelector<HTMLElement>('[data-home-entry-status]')
      if (status) status.textContent = 'That room could not be joined. Please try again.'
      return
    }
    worldStarted = true
    ...
    bindGoldShop(adapter, session, tracker)
    ...
    bindPoolDiveLauncher(panels, roomId)
...
async function renderStudyHome(
  ...
  launchWorld: (roomId: ImageRoomId, preferredInstanceId?: string | null) => Promise<void>,
) {
  ...
  ui!.innerHTML = `
    <section class="study-home" data-testid="study-home">
      <header class="home-topbar">
        <a class="home-brand" href="?view=home" aria-label="RadioTEDU Social home"><img class="radiotedu-wordmark home-brand-wordmark" src="assets/brand/radiotedu-logo-white.png" alt="" /><small>SOCIAL</small></a>
      ...
      <main>
        ...
        <div class="home-dashboard-grid">
          <section id="home-rooms" class="home-panel home-rooms-panel">
             ...
             <p class="home-room-entry-status" data-home-entry-status aria-live="assertive"></p>
           </section>
  ...
  const requestRoomEntry = async (roomId: ImageRoomId, roomTitle: string, overview?: StudyRoomOverview) => {
    const preferredInstanceId = overview
      ? await chooseRoomInstance({
          overview,
          roomTitle,
          currentInstance: adapter.roomInstance?.(roomId) ?? null,
        })
      : null
    if (preferredInstanceId === undefined) return
    await launchWorld(roomId, preferredInstanceId)
  }
  ...
  for (const card of CAMPUS_ROOM_CARDS) {
    ...
    population.textContent = overview
      ? overview.instanceCount > 1
        ? `${overview.occupancy} online · ${overview.instanceCount} rooms`
        : `${overview.occupancy}/${overview.capacity} online`
      : 'Open room'
    ...
    roomButton.addEventListener('click', () => { void requestRoomEntry(card.id, card.title, overview) })
  ...
  document.querySelector<HTMLButtonElement>('#home-enter-primary')!.addEventListener('click', () => {
    void requestRoomEntry('library', IMAGE_ROOMS.library.title, snapshot.rooms.find((room) => room.roomId === 'library'))
  })
  document.querySelector<HTMLButtonElement>('#home-enter-events')!.addEventListener('click', () => {
    void requestRoomEntry('auditorium', IMAGE_ROOMS.auditorium.title, snapshot.rooms.find((room) => room.roomId === 'auditorium'))
  })
...
function fallbackHomeSnapshot(adapter: StudyAdapter, session: StudySession): StudyHomeSnapshot {
  const capacities: Readonly<Record<StudyRoomId, number>> = Object.freeze({
    library: 60, 'chim-alan': 60, 'grass-amphitheatre': 60, 'sports-center': 60, auditorium: 60, 'learning-lab': 60,
  })
...
function renderEngineProof() {
  ui!.innerHTML = `
      <header class="game-brand" aria-label="RadioTEDU Social"><img class="radiotedu-wordmark game-brand-wordmark" src="assets/brand/radiotedu-logo-white.png" alt="" /><span>SOCIAL</span></header>
...
function renderStudyShell(session: StudySession, serverAuthoritative: boolean, entryConfig: ReturnType<typeof resolveStudyEntry>) {
  ...
  ui!.innerHTML = `
    <header class="study-bar" data-study-ui>
      <div class="study-brand" aria-label="RadioTEDU Social"><img class="radiotedu-wordmark study-brand-wordmark" src="assets/brand/radiotedu-logo-white.png" alt="" /><span class="brand-copy"><small>SOCIAL</small></span></div>
    ...
    <button id="deep-dive-launch" class="deep-dive-launch" data-study-ui type="button" aria-label="Play Deep Dive at the swimming pool" hidden>
      <span class="deep-dive-launch-icon"><i data-lucide="waves" aria-hidden="true"></i></span>
    ...
    <nav class="action-dock" data-study-ui aria-label="Game actions">
      ...
      <button id="shop-toggle" data-hud-toggle="shop" data-testid="shop-toggle" class="dock-button" type="button" aria-label="Open Exclusive Store" aria-expanded="false" aria-controls="shop-panel"><span class="dock-icon"><i data-lucide="coins" aria-hidden="true"></i></span><span class="button-label">Store</span></button>
    ...
    <aside class="world-rail" data-study-ui aria-label="Campus navigator">
      ...
      <nav class="room-tabs" role="tablist" aria-label="Study rooms">
        ...
        <button type="button" role="tab" data-room-id="chim-alan" aria-label="Giriş" aria-selected="false"><span class="room-icon"><i data-lucide="map-pin" aria-hidden="true"></i></span><span>Giriş<small>Campus entrance</small></span></button>
        <button type="button" role="tab" data-room-id="grass-amphitheatre" aria-label="Çim Alan" aria-selected="false"><span class="room-icon"><i data-lucide="sparkles" aria-hidden="true"></i></span><span>Çim Alan<small>Grass amphitheatre</small></span></button>
        <button type="button" role="tab" data-room-id="sports-center" aria-label="TEDU Swimming Pool" aria-selected="false"><span class="room-icon"><i data-lucide="waves" aria-hidden="true"></i></span><span>Swimming Pool<small>Deep Dive · server-scored</small></span></button>
        ...
        <button type="button" role="tab" data-room-id="learning-lab" aria-label="TEDU Computer Lab" aria-selected="false"><span class="room-icon"><i data-lucide="monitor" aria-hidden="true"></i></span><span>Computer Lab<small>Focused study seats</small></span></button>
      </nav>
    ...
    <aside id="shop-panel" class="hud-sheet gear-panel" data-hud-panel="shop" data-study-ui aria-label="RadioTEDU Exclusive Store" hidden>
      <header><span class="panel-heading"><i data-lucide="coins" aria-hidden="true"></i><span><small>EXCLUSIVE STORE</small><strong>Wear your campus style</strong></span></span><button data-hud-close class="close-button" type="button" aria-label="Close Exclusive Store"><i data-lucide="x" aria-hidden="true"></i></button></header>
      <p class="panel-intro">Unlock complete pixel outfits with server-verified Gold. Every item is animated for walking and sitting.</p>
      <section class="gold-economy" aria-label="Gold progress">
        ...
        <b id="shop-next-unlock" class="shop-next-unlock">Checking your next unlock…</b>
      </section>
...
function renderLockedStudy(entryConfig: ReturnType<typeof resolveStudyEntry>) {
  ui!.innerHTML = `
    <section class="study-gate" aria-labelledby="study-entry-title">
      ...
      <div class="study-entry-brand" aria-label="RadioTEDU Social World"><img class="radiotedu-wordmark study-entry-wordmark" src="assets/brand/radiotedu-logo-white.png" alt="" /><b><small>SOCIAL WORLD</small></b></div>
  ...
  const renderAuth = (mode: 'login' | 'register') => {
    ...
    stage.innerHTML = `
      ...
      <div class="study-auth-divider"><span>TEDÜ ACCOUNT</span></div>
          <button class="study-auth-tedu" type="button" data-study-tedu-login><b aria-hidden="true">T</b><span><strong>Are you a RadioTEDU crew member?</strong><small>Continue with your verified crew account</small></span></button>
...
const STORE_AVATAR_ASSET_BASE = `${import.meta.env.BASE_URL}assets/avatars/engine-proof`
const STORE_AVATAR_LAYERS = ['body', 'skin', 'hair', 'top', 'bottom', 'shoes', 'hat'] as const

function buildStoreAvatarPreview(item: GoldStoreItem): HTMLElement {
  const preview = document.createElement('span')
  preview.className = 'store-avatar-preview'
  preview.dataset.previewItem = item.id
  preview.setAttribute('aria-hidden', 'true')
  const outfit = {
    top: item.slot === 'top' ? item.id : 'radio-hoodie',
    bottom: item.slot === 'bottom' ? item.id : 'jeans',
    shoes: item.slot === 'shoes' ? item.id : 'sneakers',
    hat: item.slot === 'hat' ? item.id : null,
  }
  for (const layer of STORE_AVATAR_LAYERS) {
    const id = layer === 'top' ? outfit.top
      : layer === 'bottom' ? outfit.bottom
        : layer === 'shoes' ? outfit.shoes
          : layer === 'hat' ? outfit.hat
            : null
    if (layer === 'hat' && !id) continue
    const file = layer === 'body' || layer === 'skin' || layer === 'hair'
      ? `${layer}-idle.png`
      : `${layer}-${id}-idle.png`
    const sprite = document.createElement('i')
    sprite.className = `store-avatar-layer store-avatar-layer-${layer}`
    sprite.style.backgroundImage = `url("${STORE_AVATAR_ASSET_BASE}/${file}")`
...
function bindGoldShop(
  ...
  const render = (snapshot = studyGear.snapshot()) => {
    ...
    for (const item of GOLD_STORE_ITEMS) {
      ...
      halo.className = `store-swatch swatch ${item.swatch}`
      const price = document.createElement('b')
      price.className = 'gear-price'
      price.textContent = owned ? equipped ? 'WEARING' : 'OWNED' : `${item.price} GOLD`
      artwork.append(halo, buildStoreAvatarPreview(item), price)
      const copy = document.createElement('span')
      ...
      action.dataset.state = busy ? 'busy' : equipped ? 'equipped' : owned ? 'owned' : affordable ? 'locked' : 'unavailable'
      action.textContent = busy
        ...
        : equipped
          ...
          : owned
            ? 'Wear'
            : affordable
              ? `Buy & wear · ${item.price} Gold`
              : `Need ${item.price - snapshot.gold} more Gold`
      action.disabled = busy || equipped || (!owned && !affordable)
  ...
  grid.addEventListener('click', async (event) => {
    ...
    if (!item || pendingItemIds.has(id)) return
    const game = window.__STUDY_GAME_APP__
    if (!game) {
      message.textContent = 'The store is still getting ready. Try again in a moment.'
      return
    }
    const wasOwned = adapter.session().ownedWearableIds.includes(id)
    pendingItemIds.add(id)
    render()
    message.textContent = wasOwned
      ? `Putting on ${item.name}…`
      : `Confirming ${item.name} with the RadioTEDU server…`
    let equipped = false
    try {
      await game.equip(item.slot, id)
      equipped = true
      message.textContent = wasOwned
        ? `${item.name} is now part of your look.`
        : `${item.name} is unlocked and equipped. Your balance is ${studyGear.snapshot().gold} Gold.`
    } catch (error) {
      message.textContent = error instanceof StudyAdapterError && /INSUFFICIENT/i.test(error.code)
        ? `You do not have enough Gold for ${item.name}. Your server balance has been refreshed.`
        : `${item.name} could not be equipped. Your server inventory remains authoritative.`
    } finally {
      pendingItemIds.delete(id)
      render()
      if (equipped) {
        const equippedCard = grid.querySelector<HTMLElement>(`[data-store-item="${id}"]`)
        equippedCard?.setAttribute('data-just-equipped', 'true')
        globalThis.setTimeout(() => equippedCard?.removeAttribute('data-just-equipped'), 720)
      }
    }
  })
  ...
  const render = () => {
    ...
    for (const room of rooms) {
      ...
      card.addEventListener('click', async () => {
        if (card.dataset.joining === 'true') return
        card.dataset.joining = 'true'
        try {
          let overview: StudyRoomOverview | undefined
          try {
            overview = (await adapter.fetchHome?.())?.rooms.find((candidate) => candidate.roomId === room.id)
          } catch {
            // Room navigation remains usable if the optional home summary is temporarily unavailable.
          }
          const preferredInstanceId = overview
            ? await chooseRoomInstance({
                overview,
                roomTitle: room.title,
                currentInstance: adapter.roomInstance?.(room.id) ?? null,
              })
            : null
          if (preferredInstanceId === undefined) return
          if (preferredInstanceId) {
            await adapter.enterRoom(room.id, IMAGE_ROOMS[room.id].spawnNodeId, preferredInstanceId)
          }
          document.querySelector<HTMLButtonElement>(`[role="tab"][data-room-id="${room.id}"]`)?.click()
          panels.close()
        } catch {
          status.textContent = 'ROOM UNAVAILABLE · TRY AGAIN'
        } finally {
          delete card.dataset.joining
        }
      })
      list.append(card)
    }
...
function bindArcade(adapter: StudyAdapter) {
  ...
  host.innerHTML = `
    <article class="pool-dive" data-arcade-state="idle">
      <header>
        <span><small>SERVER-SCORED · 8 ROUNDS</small><strong>Deep Dive</strong></span>
        <b>UP TO 10 GOLD / DAY</b>
      </header>
      <p>Step onto the board, read the signal, then choose the matching direction. Timing, score and Gold are verified by the server.</p>
      <div class="pool-dive-scoreboard" aria-live="polite">
                <span><small>ROUND</small><strong data-pool-round>0 / 8</strong></span>
      ...
      <div class="pool-dive-stage" data-pool-prompt="idle">
        ...
        <div class="pool-lanes" role="group" aria-label="Choose a diving lane">
          <button type="button" data-pool-choice="left" aria-keyshortcuts="ArrowLeft A"><span aria-hidden="true">←</span><b>LEFT</b><small>A / ←</small></button>
          <button type="button" data-pool-choice="center" aria-keyshortcuts="ArrowDown S"><span aria-hidden="true">↓</span><b>CENTER</b><small>S / ↓</small></button>
          <button type="button" data-pool-choice="right" aria-keyshortcuts="ArrowRight D"><span aria-hidden="true">→</span><b>RIGHT</b><small>D / →</small></button>
        </div>
      </div>
      ...
      <button class="pool-dive-start" type="button" data-pool-start>Take your place on the board</button>
    </article>
  `
  ...
  const render = () => {
    ...
    round.textContent = session ? `${session.round} / ${session.totalRounds}` : '0 / 8'
    ...
    startButton.hidden = Boolean(session && !session.final)
  ...
  startButton.addEventListener('click', () => {
    ...
    void adapter.startPoolDive().then((next) => {
      ...
      statusMessage = code === 'SOCIAL_ARCADE_RATE_LIMITED'
        ...
        : 'Deep Dive could not start. Your Gold was not changed.'
  ...
  window.addEventListener('keydown', (event) => {
    ...
    if (document.documentElement.dataset.hudPanel !== 'events' || host.hidden) return
    const choice: SocialArcadeChoice | null = event.key === 'ArrowLeft' || event.key.toLowerCase() === 'a'
      ? 'left'
      : event.key === 'ArrowDown' || event.key.toLowerCase() === 's'
        ...
        : event.key === 'ArrowRight' || event.key.toLowerCase() === 'd'
          ...
          : null
    if (!choice) return
    const button = lanes.find((candidate) => candidate.dataset.poolChoice === choice)
    if (!button || button.disabled) return
    event.preventDefault()
    button.click()
  })
...
function bindPoolDiveLauncher(panels: BoundHudPanels, initialRoomId: StudyRoomId) {
  const launcher = document.querySelector<HTMLButtonElement>('#deep-dive-launch')
  if (!launcher) return
  let roomId = initialRoomId
  let moving = false
  const sync = () => {
    launcher.hidden = roomId !== 'sports-center'
    launcher.disabled = moving
  }
  const requestDive = () => {
    if (moving || roomId !== 'sports-center') return
    moving = true
    sync()
    window.dispatchEvent(new CustomEvent('radiotedu:study-pool-dive-request'))
  }
  const finishRequest = (available: boolean) => {
    moving = false
    sync()
    if (!available) {
      setHudMessage('DIVING BOARD UNAVAILABLE')
      return
    }
    panels.open('events')
    window.dispatchEvent(new CustomEvent('radiotedu:study-open-arcade', { detail: { activity: 'pool-dive' } }))
    globalThis.setTimeout(() => document.querySelector<HTMLButtonElement>('[data-pool-start]')?.focus())
  }
  launcher.addEventListener('click', requestDive)
  window.addEventListener('radiotedu:study-pool-dive-ready', () => finishRequest(true))
  window.addEventListener('radiotedu:study-pool-dive-failed', () => finishRequest(false))
  window.addEventListener('radiotedu:study-room-changed', (event) => {
    roomId = (event as CustomEvent<{ roomId: StudyRoomId }>).detail?.roomId ?? roomId
    if (roomId !== 'sports-center') moving = false
    sync()
  })
  sync()
}
...
function bindStudyPath(tracker: StudySessionTracker | undefined, initialRoomId: StudyRoomId) {
  ...
  window.addEventListener('radiotedu:study-open-arcade', () => setView('arcade'))
  window.addEventListener('radiotedu:study-room-changed', (event) => {
    const roomId = (event as CustomEvent<{ roomId: StudyRoomId }>).detail?.roomId
...
function bindChat(adapter: StudyAdapter, safety: IgnoredPlayerStore) {
  ...
  const syncRoomLabel = () => {
    ...
    label.textContent = `${room} · ${instance}`.toLocaleUpperCase('tr-TR')
  }
  const markRead = () => {
    unread = 0
    syncUnread()

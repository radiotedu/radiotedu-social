...
const FAKE_PRESENCE: readonly StudyPresence[] = Object.freeze([
  ...
  { userId: 'local-deniz', displayName: 'Deniz', roomId: 'chim-alan', nodeId: 'bench-west-approach', seatId: 'courtyard-bench-west', color: 0x9d6fc0 },
])
...
const LOCAL_EVENTS: readonly StudyWorldEvent[] = Object.freeze([
  ...
  Object.freeze({
    ...
    location: 'Ahmet Ersan Auditorium',
    startsAt: null,
    endsAt: null,
    rewardGold: 30,
    registered: false,
...
export class LocalStudyAdapter implements StudyAdapter {
  ...
  enterRoom(roomId: StudyRoomId, nodeId: string, _preferredInstanceId?: string | null): void {
    this.releaseSeat()
  ...
  async fetchHome(): Promise<StudyHomeSnapshot> {
    const rooms = (['library', 'chim-alan', 'grass-amphitheatre', 'sports-center', 'auditorium', 'learning-lab'] as const).map((roomId) => {

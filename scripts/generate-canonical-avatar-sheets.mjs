...
const STANDING_CELL_HEIGHT = FRAME_HEIGHT - 4
// Keep sitting and standing frames on one anatomical scale. Normalizing each
// row to the full frame made the seated head and shoulders about 20% larger,
// which read as a pasted-on character beside the Library desks.
const SEATED_CELL_HEIGHT = 76
...
async function normalizedCell(source, extract, cellHeight) {
  ...
  const frame = await sharp({
    ...
    .composite([{ input: trimmed, left: 2, top: FRAME_HEIGHT - cellHeight - 2 }])
    .png()
    .toBuffer()
...
async function referenceFrames() {
  ...
  for (const [rowIndex, row] of rows.entries()) {
    const frames = []
    const cellHeight = rowIndex === 0 ? STANDING_CELL_HEIGHT : SEATED_CELL_HEIGHT

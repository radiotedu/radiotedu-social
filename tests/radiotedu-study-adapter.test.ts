...
describe('RadioTEDUStudyAdapter', () => {
  ...
  it('sends an explicit room selection through the existing join contract', async () => {
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce(success({
        instance: { id: 'library-1', roomId: 'library', number: 1, occupancy: 60, capacity: 60, preferredInstanceFull: false },
      }))
      .mockResolvedValueOnce(success({
        instance: { id: 'library-2', roomId: 'library', number: 2, occupancy: 4, capacity: 60, preferredInstanceFull: false },
      }))
    const adapter = createAdapter(fetchImpl)

    await adapter.enterRoom('library', 'bottom-center-aisle')
    await adapter.enterRoom('library', 'bottom-center-aisle', 'library-2')

    expect(fetchImpl).toHaveBeenCalledTimes(2)
    expect(JSON.parse(fetchImpl.mock.calls[1]![1].body)).toMatchObject({
      roomId: 'library', preferredInstanceId: 'library-2', nodeId: 'bottom-center-aisle',
    })
    expect(adapter.roomInstance?.('library')).toMatchObject({ id: 'library-2', number: 2 })
  })
  ...
  it('loads and validates the authoritative study home and leaderboard', async () => {
    ...
    const fetchImpl = vi.fn().mockResolvedValueOnce(success({
      ...
      rooms: [
        ...
        { roomId: 'chim-alan', occupancy: 4, capacity: 60, instanceCount: 1 },
        { roomId: 'grass-amphitheatre', occupancy: 0, capacity: 60, instanceCount: 1 },
        { roomId: 'sports-center', occupancy: 3, capacity: 60, instanceCount: 1 },
        { roomId: 'auditorium', occupancy: 6, capacity: 60, instanceCount: 1 },
        { roomId: 'learning-lab', occupancy: 4, capacity: 60, instanceCount: 1 },
      ],
    ...
    expect(home.activePlayers).toBe(91)
    expect(home.rooms).toHaveLength(6)
    expect(home.rooms[0]).toMatchObject({ roomId: 'library', occupancy: 74, capacity: 60, instanceCount: 2 })

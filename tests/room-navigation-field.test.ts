...
const ROOM_HASHES = Object.freeze({
  ...
  'tedu-orta-bahce-chillin-cafe-wide-r4.png': 'ac46adbe1dd91f26b82fb402abcc86580ce9dd61c0785403578e410410ac8e0c',
  'tedu-sports-center-wide.png': '4ad9bb8bbeb6a2a5ff55d15ef4a99b25872f4092d2fbf1ddc5e371f0c12e9313',
  'tedu-swimming-pool-wide-r2.png': 'de48f7ffef84dbd487cd4e2ac0596c10d6c5e31846baf2408a4a51ea82439660',
...
describe('room navigation fields', () => {
    ...
    it(`${room.id} exposes connected collision-safe floor routes`, () => {
      ...
      for (const [z, nodes] of nodesByLayer) {
        ...
        for (let index = 1; index < route.length; index += 1) {
          expect(
            field.segmentIsWalkable(route[index - 1]!, route[index]!, z),
            `${room.id} z=${z} unsafe smoothed segment ${JSON.stringify(route[index - 1])} -> ${JSON.stringify(route[index])}`,
          ).toBe(true)
        }
  ...
  it('keeps Auditorium movement on the foreground floor and diagonal aisles', () => {
    ...
    for (const [label, x, y] of [
      ['foreground floor', 50, 82],
      ['left stage aisle', 40, 62],
      ['centre lit aisle', 70, 60],
      ['upper lit aisle', 78, 42],
      ['right stepped aisle', 93, 48],
    ] as const) {
      expect(field.isWalkable(point(x, y)), label).toBe(true)
    }
    for (const [label, x, y] of [
      ['centre seating bank', 60, 55],
      ['right seating bank', 80, 50],
      ['foreground wall', 50, 94],
      ['left wall', 2, 22],
      ['rear wall', 50, 12],
    ] as const) {
  ...
  it('plans the same collision-safe Library route cooperatively without blocking one task', async () => {
    const room = IMAGE_ROOMS.library
    const field = new RoomNavigationField({
      width: room.image.width,
      height: room.image.height,
    ...
    const spawnNode = room.nodes.find((node) => node.id === room.spawnNodeId)!
    const spawn = roomPointToPixel(room, spawnNode)
    const target = { x: 596.33, y: 655.31 }
    const route = await field.findPathAsync(spawn, target, 0, { budgetMs: 3 })

    expect(route.length).toBeGreaterThan(1)
    expect(route.at(-1)!.x).toBeCloseTo(target.x, 5)
    expect(route.at(-1)!.y).toBeCloseTo(target.y, 5)
    for (let index = 1; index < route.length; index += 1) {
      expect(field.segmentIsWalkable(route[index - 1]!, route[index]!)).toBe(true)
    }
  })
})

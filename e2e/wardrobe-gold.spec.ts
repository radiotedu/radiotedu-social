...
test('every wearable renders, equips, persists, walks, and sits on desktop and mobile', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: 'TEDU Swimming Pool' }).click()
...
test('server-authoritative Gold purchases are deduplicated and update all paid wearables', async ({ page }, testInfo) => {
  ...
  for (const [id, slot] of [['beanie', 'hat'], ['boots', 'shoes'], ['black-cargos', 'bottom'], ['radiotedu-tee', 'top'], ['varsity-jacket', 'top']] as const) {
    await expect(page.locator(`[data-preview-item="${id}"] .store-avatar-layer-${slot}`)).toHaveCSS('background-image', new RegExp(`${slot}-${id}-idle`))
  }

  await page.locator('[data-store-item-id="varsity-jacket"]').evaluate((button: HTMLButtonElement) => {
    button.click()
    button.click()
  })
  await expect(page.locator('#point-balance')).toHaveText('205')
  await expect(page.locator('[data-store-item="varsity-jacket"]')).toHaveAttribute('data-equipped', 'true')
  await expect(page.locator('[data-store-item-id="varsity-jacket"]')).toHaveText('Wearing')
  ...
  for (const [id, expectedGold] of [['black-cargos', '145'], ['boots', '95'], ['beanie', '60'], ['radiotedu-tee', '15']] as const) {
    const button = page.locator(`[data-store-item-id="${id}"]`)
    await expect(button).toContainText('Buy & wear')
    await button.click()
    await expect(page.locator('#point-balance')).toHaveText(expectedGold)
    await expect(page.locator(`[data-store-item="${id}"]`)).toHaveAttribute('data-equipped', 'true')
  }

  await page.locator('[data-store-item-id="varsity-jacket"]').click()
  await expect(page.locator('#point-balance')).toHaveText('15')
  await expect.poll(async () => (await page.evaluate(() => window.__STUDY_GAME_APP__.snapshot())).topId).toBe('varsity-jacket')

...
test('captures room scale and chair proportions after real mouse movement', async ({ page }, testInfo) => {
  ...
  await page.getByRole('tab', { name: 'Computer Lab' }).click()
  ...
  expect(floor, 'Computer Lab needs a visible floor point near the chairs').not.toBeNull()
  ...
  for (const roomId of ['chim-alan', 'auditorium', 'sports-center'] as const) {
    await page.getByRole('tab', { name: roomId === 'chim-alan' ? 'Giriş' : roomId === 'auditorium' ? 'Auditorium' : 'Swimming Pool' }).click()

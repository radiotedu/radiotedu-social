...
test('presents an immersive game HUD and room-scoped working chat', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Open Exclusive Store' })).toBeVisible()
  ...
  await expect(page.getByTestId('navigator-room-list').locator('.navigator-room-card')).toHaveCount(6)
  ...
  await expect(page.locator('#room-arrival-title')).toHaveText('Giriş')
  ...
  await expect(page.locator('#chat-room-label')).toContainText('GİRİŞ')

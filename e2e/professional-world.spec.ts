...
test('professional campus world evidence', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#room-title')).toHaveText('Giriş')
  ...
  await expect(page.locator('#room-title')).toHaveText('TEDU Swimming Pool')
  const sportsEntrance = await page.evaluate(() => window.__STUDY_GAME_APP__.snapshot())
  await page.evaluate(() => window.__STUDY_GAME_APP__.walkToNode('rear-center'))
  await expect.poll(async () => (await page.evaluate(() => window.__STUDY_GAME_APP__.snapshot())).nodeId, {
    timeout: 20_000,
  ...
  await expect(page.locator('#room-title')).toHaveText('TEDU Computer Lab')
  ...
  await capture(page, testInfo, '07-computer-lab-study')

  await page.getByRole('tab', { name: 'Auditorium' }).click()
  await expect(page.locator('html')).toHaveAttribute('data-room-id', 'auditorium')
  await expect(page.locator('#room-title')).toHaveText('Ahmet Ersan Auditorium')
  await expect(page.locator('html')).toHaveAttribute('data-auditorium-screen', 'radiotedu')
  await capture(page, testInfo, '08-auditorium')

  await page.waitForTimeout(1_200)
  await capture(page, testInfo, '09-auditorium-event-screen')
  ...
  await capture(page, testInfo, '10-auditorium-study')

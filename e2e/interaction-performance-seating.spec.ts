...
type LongTaskSample = Readonly<{ startTime: number; duration: number }>
...
test('real chair clicks start a seated study session in every seat-bearing room', async ({ page }, testInfo) => {
  ...
  for (const roomId of ['library', 'chim-alan', 'grass-amphitheatre', 'auditorium', 'learning-lab'] as const) {
...
test('rapid real clicks stay responsive and settle on the latest walk intent', async ({ page }, testInfo) => {
  ...
  // Target discovery exercises a test-only navigation debug surface and can
  // allocate thousands of floor samples. Start measuring after that setup so
  // the metric covers the real pointer burst and route planning only.
  await page.evaluate(() => {
    const target = window as Window & { __studyLongTasks?: LongTaskSample[]; __studyLongTaskObserver?: PerformanceObserver }
    ...
    target.__studyLongTaskObserver = new PerformanceObserver((list) => {
      target.__studyLongTasks!.push(...list.getEntries().map((entry) => ({
  ...
  const clickWindowStartedAt = await page.evaluate(() => performance.now())
  const startedAt = Date.now()
  for (const target of targets) await tap(page, testInfo, target)
  const dispatchMs = Date.now() - startedAt
  const clickWindowEndedAt = await page.evaluate(() => performance.now())
  await page.waitForTimeout(300)
  const longTasks = await page.evaluate(() => {
    return (window as Window & { __studyLongTasks?: LongTaskSample[] }).__studyLongTasks ?? []
  })

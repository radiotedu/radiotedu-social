...
test('real canvas clicks reach exact collision-safe floor points in every room', async ({ page }, testInfo) => {
  ...
  for (const roomId of ['library', 'chim-alan', 'grass-amphitheatre', 'sports-center', 'auditorium', 'learning-lab'] as const) {
...
test('redirecting mid-walk replans from the live position without backtracking', async ({ page }, testInfo) => {
  ...
  // A collision-safe A* route may initially move sideways around the pool
  // shell. Permit one four-cell clearance bend while still rejecting the much
  // larger reversal that occurs when the obsolete route keeps running.
  const redirectTolerance = await page.evaluate(() => window.__STUDY_GAME_APP__.navigation().cellSize * 4)
  expect(greatestDistance).toBeLessThanOrEqual(startingDistance + redirectTolerance)
})
